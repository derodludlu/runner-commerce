...
const inputDir =
  process.env.WHATSAPP_INGEST_INPUT_DIR ?? './whatsapp-inbox/pending';
const processedDir =
  process.env.WHATSAPP_INGEST_PROCESSED_DIR ?? './whatsapp-inbox/processed';
const failedDir =
  process.env.WHATSAPP_INGEST_FAILED_DIR ?? './whatsapp-inbox/failed';
...
async function main() {
  ...
  console.log(
    `WhatsApp ingest worker watching ${inputDir} -> ${backendUrl}/whatsapp-imports/webhook`,
  );
...
async function processPendingFiles() {
  ...
  for (const file of files) {
    ...
    try {
      const batch = parseBatch(await readFile(fullPath, 'utf8'), file);
      const result = await postBatch(batch);
      await archiveFile(fullPath, processedDir);
      console.log(
        `Imported ${file}: queued=${result.queued} parsed=${result.parsed} needsReview=${result.needsReview}`,
      );
    } catch (error) {
...
function parseBatch(raw: string, fileName: string): WhatsAppBatch {
  const payload = JSON.parse(raw) as Partial<WhatsAppBatch>;
...
async function postBatch(batch: WhatsAppBatch) {
...
async function archiveFile(sourcePath: string, destinationDir: string) {
  const stampedName = `${Date.now()}-${basename(sourcePath)}`;
  await rename(sourcePath, join(destinationDir, stampedName));
}
